...
// ── Legacy PRO grants (deprecated) ─────────────────────────

/**
 * One-way bootstrap of NOTIFY_PRO_USER_IDS into user_contacts.tier.
 *
 * **The column is the source of truth; the environment variable is not.** PRO
 * used to be granted by an env var, which meant it could only be changed by a
 * redeploy, was invisible from inside the app, and quietly disagreed with the
 * `tier` column that everything else already read. The admin dashboard now
 * owns the decision.
 *
 * This exists so switching over does not silently downgrade whoever the env
 * var was carrying — it copies those ids in once, and then gets out of the way:
 * a row whose `tier_source` is 'admin' is never touched, so a downgrade made in
 * the dashboard is not undone on the next cron run by a stale variable nobody
 * remembers setting. Delete this function, and the variable, once the
 * dashboard shows the same people as PRO.
 */
export async function syncLegacyProGrants(clerkUserIds: string[]): Promise<number> {
  if (!hasDb || clerkUserIds.length === 0) return 0;

  try {
    const rows = await sql`
      INSERT INTO user_contacts (clerk_user_id, tier, tier_source, tier_updated_at, tier_set_by)
      SELECT u, 'pro', 'legacy_env', now(), 'env:NOTIFY_PRO_USER_IDS'
      FROM unnest(${clerkUserIds}::text[]) AS u
      ON CONFLICT (clerk_user_id) DO UPDATE SET
        tier            = 'pro',
        tier_source     = 'legacy_env',
        tier_updated_at = now(),
        tier_set_by     = 'env:NOTIFY_PRO_USER_IDS',
        updated_at      = now()
      WHERE user_contacts.tier_source <> 'admin'
        AND user_contacts.tier <> 'pro'
      RETURNING clerk_user_id`;
    return rows.length;
  } catch {
    // db/017_admin.sql not applied yet — the tier column alone still works,
    // and a missing bootstrap is not worth failing a notification run over.
    return 0;
  }
}
